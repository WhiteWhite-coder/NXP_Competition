...
uint16 MaxMotoring = 500,MinMotoring = 0;   //经调整后电机驱动的PWM占空比最大值是1000
...
void Motorcontrol_Left()
{
    Motor_error_Left = (int16)(Motor_P*speed_e_left                            //P*（E（k）-E（k-1））
                             + Motor_I*(speed_e_left + speed_last_e_left)      //I*（E（k）-E（k-1））
                             + Motor_D*speed_ec_left);                            //D*（E（k）-2E（k-1）+E（k-2））
    Motor_Last_Left = Motor_Ctrl_Left;   //上次电机输出
    Motor_Ctrl_Left = (int16)(Motor_Last_Left +  Motor_error_Left);   //PID算得的本次电机输出
    if(Motor_Ctrl_Left>=MaxMotoring)   Motor_Ctrl_Left = MaxMotoring;   //电机最大PWM限幅
    if(Motor_Ctrl_Left<=MinMotoring)   Motor_Ctrl_Left = MinMotoring;   //电机最小PWM限幅
}

void Motorcontrol_Right()
{
    Motor_error_Right = (int16)(Motor_P*speed_e_right                               //P*（E（k）-E（k-1））
                              + Motor_I*(speed_e_right + speed_last_e_right)        //I*（E（k）-E（k-1）） 
                              + Motor_D*speed_ec_right);                          //D*（E（k）-2E（k-1）+E（k-2））
    Motor_Last_Right = Motor_Ctrl_Right;//上次电机输出 
    Motor_Ctrl_Right = (int16)(Motor_Last_Right +  Motor_error_Right);//PID算得的本次电机输出
    if(Motor_Ctrl_Right>=MaxMotoring)   Motor_Ctrl_Right = MaxMotoring;//电机最大PWM限幅
    if(Motor_Ctrl_Right<=MinMotoring)   Motor_Ctrl_Right = MinMotoring;//电机最小PWM限幅
}
